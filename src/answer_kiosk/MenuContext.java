...
class MenuContext {
    private Map<String, List<Menu2>> menus;	// 메뉴
    ...
    private void initializeMenuItems() {
        List<Menu2> mainMenus = new ArrayList<>();
        mainMenus.add(new Menu2("Burgers", "앵거스 비프 통살을 다져만든 버거"));
        mainMenus.add(new Menu2("Frozen Custard", "매장에서 신선하게 만드는 아이스크림"));
        mainMenus.add(new Menu2("Drinks", "매장에서 직접 만드는 음료"));
        mainMenus.add(new Menu2("Beer", "뉴욕 브루클린 브루어리에서 양조한 맥주"));

        List<Menu2> orderMenus = new ArrayList<>();
        orderMenus.add(new Menu2("Order", "장바구니를 확인 후 주문합니다."));
        orderMenus.add(new Menu2("Cancel", "진행중인 주문을 취소합니다."));
        orderMenus.add(new Menu2("Order List", "대기/완료된 주문목록을 조회합니다."));
     ...
     * @return 조회된 메뉴 목록
     */
    public List<Menu2> getMenus(String key) {
    ...
    public void addMenu(String key, String description) {
        menus.get("Main").add(new Menu2(key, description));
    ...
    public String getMainMenuName(int id) {
        List<Menu2> mainMenus = menus.get("Main");
        for (Menu2 mainMenu : mainMenus) {
            if (mainMenu.id == id) {
